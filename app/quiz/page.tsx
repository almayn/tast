...
import { FaSnapchat, FaShareAlt, FaCamera } from 'react-icons/fa'; // أضف أيقونة الكاميرا
import PrizesModal from '.././components/PrizesModal';
import html2canvas from 'html2canvas'; // استيراد مكتبة html2canvas
...
export default function Home() {
  ...
  // دالة لتصوير الشاشة وتصديرها كصورة
  const handleScreenshot = () => {
    const element = document.getElementById('screenshot-target'); // العنصر الذي تريد تصويره
    if (element) {
      html2canvas(element).then((canvas) => {
        const image = canvas.toDataURL('image/png'); // تحويل Canvas إلى صورة
        const link = document.createElement('a');
        link.href = image;
        link.download = 'مسابقة_الماس_الرمضانية.png'; // اسم الملف المحفوظ
        link.click();
      });
    }
  };
  ...
  return (
    <div dir="rtl" className="max-w-md mx-auto mt-2 p-6 bg-white shadow-lg rounded-lg text-center" id="screenshot-target">
      ...
      <div className="flex justify-center gap-4 mt-6">
        ...
        <button
          ...
          className="bg-purple-500 text-white px-4 py-2 rounded-md hover:bg-purple-600 transition flex items-center gap-2 shadow-md w-32 justify-center"
        >
          <FaCamera size={24} />
          <span>تصوير</span>
        </button>
